    ...
    <div className="relative z-10 max-w-4xl mx-auto animate-fade-in">
      ...
      <p className="text-2xl md:text-3xl font-medium leading-relaxed drop-shadow-md">
        "لقمتنا هنية.. والجمعة دافية" <br/> 
        <span className="text-orange-200 text-xl font-normal">نكهة البصرة الأصلية في كل صحن</span>
      </p>
    </div>
  </header>
);
...
const FoodGallery: React.FC = () => (
  <section className="relative max-w-7xl mx-auto py-24 px-6 z-10">
    <div className="text-center mb-20">
      <span className="text-orange-600 font-bold tracking-widest uppercase text-sm mb-2 block">قائمة الطعام</span>
      ...
      <p className="text-stone-600 text-xl max-w-2xl mx-auto leading-relaxed">
        من القيمة النجفية المعدلة للبركر المشوي على الأصول، كل طبق نحضره بشغف ليرضي ذوقكم.
      </p>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
      {MENU_ITEMS.map((item, index) => (
        <div key={index} className="food-card bg-white/90 backdrop-blur-sm rounded-[2rem] shadow-xl overflow-hidden transition-all duration-500 hover:shadow-2xl group border border-white/50">
          <div className="h-80 overflow-hidden relative">
            ...
            <div className="absolute bottom-8 right-8 text-white">
              <h3 className="text-4xl font-black drop-shadow-xl">{item.title}</h3>
            </div>
          </div>
          <div className="p-10">
            <p className="text-stone-700 text-xl leading-relaxed text-right font-medium">{item.description}</p>
          </div>
        </div>
      ))}
    </div>
...
const InfoSection: React.FC = () => (
  <section className="relative py-24 px-6 z-10">
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
      ...
      <div className="bg-white/95 backdrop-blur-md p-12 rounded-[3rem] shadow-2xl border-r-8 border-orange-500 flex flex-col justify-center">
        ...
        <div className="mt-10 p-6 bg-stone-50 rounded-2xl border-dashed border-2 border-stone-200 text-stone-500 italic text-lg">
          ننتظركم لتقديم أشهى الوجبات في أجواء بصرية دافئة.
        </div>
      </div>

      {/* Dedicated Social Box */}
      <div className="bg-stone-900/95 backdrop-blur-md p-12 rounded-[3rem] shadow-2xl text-white flex flex-col justify-center border-l-8 border-yellow-500">
        <div className="flex items-center mb-8">
